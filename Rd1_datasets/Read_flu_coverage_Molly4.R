...
setwd("//Users/mollyblair/Desktop/Rstudio/Flu Project")
...
#dates=c(as.Date("2021-07-15"),as.Date("2021-08-01"),as.Date("2021-08-15"),as.Date("2021-09-15"),unique(flu2021$Date) )
...
library(devtools)
install_github("HopkinsIDD/uscovid19vacc@main")
...
pop.k0_4=state_pop_data %>% dplyr::filter(AGE<5 & SEX == 0 & NAME != "United States") %>% 
  group_by(NAME) %>% 
  mutate(w_pop=case_when(AGE==0 ~ POPEST2020_CIV*0.5,
...
pop.k0_17=state_pop_data %>% dplyr::filter(AGE<18 & SEX == 0 & NAME != "United States") %>% 
  group_by(NAME) %>% 
  mutate(w_pop=case_when(AGE==0 ~ POPEST2020_CIV*.5,
                         AGE>0 ~ as.numeric(POPEST2020_CIV))) %>% 
  dplyr::summarise(pop=round(sum(w_pop))) %>%
  left_join(states, by = c("NAME" = "region"))  %>% 
...
pop.a65=state_pop_data %>% dplyr::filter(AGE>64 & SEX == 0 & NAME != "United States") %>% 
  group_by(NAME) %>% 
  mutate(w_pop=POPEST2020_CIV) %>% 
  dplyr::summarise(pop=round(sum(w_pop))) %>%
  left_join(states, by = c("NAME" = "region"))  %>% 
...
ggsave("FluCoverageAllstates_fluRd1_Sc_C_D.png", p, width=14, height=12)
